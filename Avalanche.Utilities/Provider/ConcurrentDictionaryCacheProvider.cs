...
public abstract class ConcurrentDictionaryCacheProvider : IProvider, IProviderStack, ICache, IDecoration
{
    /// <summary>Constructor</summary>
    static readonly ConstructorT2<Delegate, Delegate?, ConcurrentDictionaryCacheProvider> delegateCtor = new(typeof(ConcurrentDictionaryCacheProvider<,>));
    /// <summary>Constructor</summary>
    static readonly ConstructorT2<IProvider, Delegate?, ConcurrentDictionaryCacheProvider> providerCtor = new(typeof(ConcurrentDictionaryCacheProvider<,>));
    /// <summary>Create</summary>
    public static ConcurrentDictionaryCacheProvider Create(Type keyType, Type valueType, Delegate createFunc, Delegate? toCacheCriteria = default) => delegateCtor.Create(keyType, valueType, createFunc, toCacheCriteria);
    /// <summary>Create</summary>
    public static ConcurrentDictionaryCacheProvider Create(Type keyType, Type valueType, IProvider createProvider, Delegate? toCacheCriteria = default) => providerCtor.Create(keyType, valueType, createProvider, toCacheCriteria);
    /// <summary>Create</summary>
    public static ConcurrentDictionaryCacheProvider Create(IProvider createProvider, Delegate? toCacheCriteria = default) => providerCtor.Create(createProvider.Key, createProvider.Value, createProvider, toCacheCriteria);
...
/// <summary>Cache concurrent use.</summary>
/// <remarks>Uses <see cref="ConcurrentDictionary{Key,Value}"/></remarks>
public class ConcurrentDictionaryCacheProvider<TKey, TValue> :
    ConcurrentDictionaryCacheProvider,
    ...
    where TKey : notnull
{
    ...
    public TValue this[TKey key]
    {
        get
        {
            ...
            if (map.TryGetValue(key, out TValue? existing)) return existing;
            // Create new
            TValue newValue = createProvider[key];
            // Try cache
            TValue value = map.GetOrAdd(key, newValue);
            // Return result
            return value;
        }
    }
    ...
    /// <summary>Map</summary>
    protected ConcurrentDictionary<TKey, TValue> map;
    /// <summary>Map</summary>
    public new ConcurrentDictionary<TKey, TValue> Map => map;
    /// <summary>Map</summary>
    ...
    public ConcurrentDictionaryCacheProvider(IProvider<TKey, TValue> createProvider, Func<TKey, bool>? toCacheCriteria = default)
    {
    ...
    /// <param name="toCacheCriteria">Evaluates whether key is to be cached or not.</param>
    /// <remarks>Note that <paramref name="toCacheCriteria"/> may be invoked multiple times in concurrent scenario and value let go</remarks>
    public ConcurrentDictionaryCacheProvider(IProvider<TKey, TValue> createProvider, ConcurrentDictionary<TKey, TValue> map, Func<TKey, bool>? toCacheCriteria = default)
    {
        this.createProvider = createProvider ?? throw new ArgumentNullException(nameof(createProvider));
        this.sources = new[] { createProvider };
    ...
    /// <param name="toCacheCriteria">Evaluates whether key is to be cached or not.</param>
    /// <remarks>Note that <paramref name="toCacheCriteria"/> may be invoked multiple times in concurrent scenario and value let go</remarks>
    public ConcurrentDictionaryCacheProvider(IProvider<TKey, TValue> createProvider, IEqualityComparer<TKey> comparer, Func<TKey, bool>? toCacheCriteria = default)
    {
        this.createProvider = createProvider ?? throw new ArgumentNullException(nameof(createProvider));
        this.sources = new[] { createProvider };
        this.map = new ConcurrentDictionary<TKey, TValue>(comparer);
        this.toCacheCriteria = toCacheCriteria;
    }
    ...
    public override bool TryGetValue(object keyObject, out object value)
    {
        ...
        // Try getting existing
        if (map.TryGetValue(key, out TValue? value1)) { value = value1!; return true; }
        // Create new
        if (!createProvider.TryGetValue(key, out value1)) { value = default!; return false; }
        // Cache
        value1 = map.GetOrAdd(key, value1);
        // Return result
        value = value1!;
        return true;
    }
